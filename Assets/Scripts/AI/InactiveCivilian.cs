...
public class InactiveCivilian : MonoBehaviour
{
	...
	void Update ()
    {
        if (!rescued)
            ...
            {
                int mask = ~(1 << 9);
                if (Vector3.Distance(transform.position, GameObjectManager.instance.players[i].gameObject.transform.position) <= range)
                {
                    if (!Physics.Linecast(transform.position, GameObjectManager.instance.players[i].gameObject.transform.position, mask))
                    {
